...
def write_positive_controls(eo_result: dict, sex_result: dict, out_path: Path,
    ...
    guarded_write(out_path, text, allow_overwrite=True)
...
def render_benchmark_reproduction(result: dict) -> str:
    ...
    lines = [
        ...
        "",
        "## Methodological gap (independent of this run's outcome)",
        "",
        "This project's baseline feature set is 7 relative-band-powers + a 1/f fit, "
        "ROI-averaged over the entire MSIT recording. The published 0.58 used multitaper "
        "time-frequency area features computed per condition, with t-test+FDR subspace "
        "feature selection — a substantially richer feature-engineering and selection "
        "pipeline, on a different cohort. That difference in feature richness is a "
        "characterized methodological gap, not evidence about pipeline correctness, and it "
        "holds regardless of the number reported above.",
        "",
        "> **Note.** The one-off diagnostic investigation accompanying the originally "
        "> delivered run (data-sanity checks, two-ROI univariate screen, alternate-classifier "
        "> comparison) is recorded in the delivered artifact for that run and is deliberately "
        "> *not* regenerated here: those figures describe one specific execution and would be "
        "> false if reproduced verbatim against different inputs (phase_7.md §3).",
        "",
    ]
    return "\n".join(lines)
...
def write_results(primary: dict, secondary: dict, eo_result: dict, sex_result: dict,
    ...
    guarded_write(html_path, _markdown_to_self_contained_html(text), allow_overwrite=True)
...
def render_control_diagnostic(ladder_result: dict, significance_alpha: float = 0.05) -> str:
    ...
    lines = [
        ...
        "primary target. It cannot change the primary verdict, which is frozen and reported "
        "separately. Its only output is which of the two sentences below the final report "
