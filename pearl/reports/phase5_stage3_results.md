# Phase 3 Results — VERDICT: POSITIVE

Primary analysis (PSWT features on resting-state, `binary_risk_vs_none`, n=64): **AUC 0.651**, 95% CI [0.353, 0.764], permutation p=0.0410.
...
- AUC: **0.651**
- 95% bootstrap CI: [0.353, 0.764]
- Permutation p-value (full pipeline re-run per permutation): 0.0410
- Spread across the 10 repeats: [0.581, 0.713]
...
| Nuisance-only | 0.557 | [0.321, 0.746] | this phase, §2b |
| Published benchmark | 0.580 | — | Li et al. 2025, MSIT (not rest) |
| **PSWT features (primary)** | **0.651** | [0.353, 0.764] | this analysis |
| Features + nuisance | 0.638 | [0.358, 0.787] | this analysis |

## 3. Positive control outcomes

- 1a (eyes-open vs eyes-closed): AUC 0.849 (PASS, floor 0.80)
- 1b (sex classification): AUC 0.465 (FAIL, floor 0.65)
- Full detail: `reports/phase3_positive_controls.md`

## 4. Benchmark reproduction outcome

- MSIT baseline features vs risk_vs_none: AUC 0.490 (verdict: NEAR_BENCHMARK)
...
|---|---|---|---|
| baseline_rest | 0.506 | 0.4815 | 0.9630 |
| pswt_plus_baseline_rest | 0.616 | 0.0879 | 0.2637 |
| baseline_msit | 0.490 | 0.5375 | 0.9630 |
...
|:---------------------------------------|----------------------:|-------------------:|:---------|
| harmonic_amplitude_profile_h2_median   |                     1 |               0.66 | True     |
| harmonic_amplitude_profile_h2_iqr      |                     1 |               0.92 | True     |
| harmonic_amplitude_profile_h3_median   |                     1 |               0.7  | True     |
| harmonic_amplitude_profile_h3_iqr      |                     1 |               1    | True     |
| harmonic_amplitude_profile_h4_median   |                     1 |               0.52 | True     |
| harmonic_amplitude_profile_h4_iqr      |                     1 |               0.7  | True     |
| harmonic_amplitude_profile_h5_median   |                     1 |               1    | True     |
| harmonic_amplitude_profile_h5_iqr      |                     1 |               0.92 | True     |
| harmonic_to_interharmonic_ratio_median |                     1 |               0.98 | True     |
| harmonic_to_interharmonic_ratio_iqr    |                     1 |               0.54 | True     |
| cycle_to_cycle_variability_median      |                     1 |               1    | True     |
| cycle_to_cycle_variability_iqr         |                     1 |               1    | True     |
| period_variability_median              |                     1 |               1    | True     |
| period_variability_iqr                 |                     1 |               1    | True     |
| rise_decay_ratio_median                |                     1 |               0.84 | True     |
| rise_decay_ratio_iqr                   |                     1 |               0.98 | True     |
| peak_trough_sharpness_ratio_median     |                     1 |               0.66 | True     |
| peak_trough_sharpness_ratio_iqr        |                     1 |               0.78 | True     |
...
- **Power** (Amendment 2 §0f): at 39 vs 25, the 95% CI on an AUC estimate is expected to be roughly ±0.13–0.15 wide; the observed CI width (0.411) is consistent with this a priori expectation.
...
- **Cross-subject generalisation caveat**: positive control 1b (sex) landed at chance (AUC 0.465), so this null cannot fully rule out that the feature pipeline simply doesn't generalise across subjects for any target, not specifically genotype.
- **h5 duration confound** (§4 leakage checklist): the two harmonic-5 columns, selected in 100% of folds, correlate strongly with recording duration among their small non-missing subset — a transparency caveat on feature stability, not a leakage bug (permutation null centred at 0.489, structurally clean).
